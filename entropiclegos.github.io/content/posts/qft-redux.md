...
Expansion of this exponential yields how the interaction terms influence time-evolution in successive orders of approximation. Note, however, that all terms except the first (which is the identity) involves time-ordering. First-order perturbation theory just involves the interaction Hamiltonian (appropriately modified per the interaction picture prescription), which makes it easier to deal with, but at second and higher orders of perturbation theory, we still haven't solved the issue of time ordering. 
